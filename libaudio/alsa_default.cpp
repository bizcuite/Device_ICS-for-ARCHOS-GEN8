...
static status_t s_open(alsa_handle_t *handle, uint32_t devices, int mode, uint32_t channels)
{
    ...
    s_close(handle);
    direction(handle);
    snd_pcm_stream_name(direction(handle));

    LOGI("open called for devices %08x in mode %d...", devices, mode);
...
/*static status_t s_standby(alsa_handle_t *handle)
{
    ...
    //return NO_ERROR;
    //Changed standby to the same as in omap3 alsa module source to prevent the clicking noise on gen8, it seems also to improve deep sleep...
    status_t err = NO_ERROR;
    snd_pcm_t *h = handle->handle;
    handle->handle = 0;
    LOGD("In omap3 standby\n");
    if (h) {
        snd_pcm_drain(h);
        err = snd_pcm_close(h);
        if (err)
            LOGE("Failed closing ALSA stream: %s", snd_strerror(err));
        LOGD("called drain&close\n");
    }
